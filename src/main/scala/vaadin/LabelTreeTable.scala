...
object LabelTreeTable {
  ...
  val Actions: Array[Action] = Array(ActionAddChild, ActionAddSibling, ActionEdit, ActionDelete)
}

class LabelTreeTable(objs: List[ContentTag]) extends TreeTable {
  val tree = this
  var objMap: HashMap[AnyRef, ContentTag] = HashMap.empty
  ...
  addContainerProperty("abbr", classOf[String], "Abbreviation")
  addContainerProperty("foreground", classOf[String], "Foreground")
  addContainerProperty("background", classOf[String], "Background")
  addContainerProperty("position", classOf[String], "Position")
  
  addContentTags(objs, null)
  
  setSizeFull()
  setColumnExpandRatio("name", 0.6f)
  setColumnExpandRatio("abbr", 0.4f)
  setImmediate(true)
  setItemCaptionPropertyId("name")
  setItemCaptionMode(AbstractSelect.ITEM_CAPTION_MODE_PROPERTY)
  setSelectable(true)
  addStyleName("striped")
  setVisibleColumns(List("name", "abbr").toArray)
  setColumnHeaderMode(Table.COLUMN_HEADER_MODE_EXPLICIT)
  setColumnHeader("name", "Name")
  setColumnHeader("abbr", "Abbreviation")
  ...
  addActionHandler(new Action.Handler() {
    def getActions(target: AnyRef, sender: AnyRef): Array[Action] = {
      LabelTreeTable.Actions
    }
  
    def handleAction(action: Action, sender: AnyRef, target: AnyRef) {
      val item = getItem(target)
      action match {
        case addChild if (addChild == LabelTreeTable.ActionAddChild) => 
          getWindow().addWindow(new NewLabelWindow(tree, target))
        case addSibling if (addSibling == LabelTreeTable.ActionAddSibling) => 
          getWindow().addWindow(new NewLabelWindow(tree, tree.getParent(target)))
        case editItem if (editItem == LabelTreeTable.ActionEdit) => 
          getWindow().addWindow(new EditLabelWindow(tree, item, target, objMap(target)))
        case delItem if (delItem == LabelTreeTable.ActionDelete) => 
  ...
  private class ConfirmDeletionWindow(tree: LabelTreeTable, itemId: AnyRef) extends Window with ClickListener {
    setWidth("360px")
    setHeight("144px")
    setCaption("Are you sure?")
    center()
    
    val lbl = new Label("This action will delete this label and all child "+
      "labels and will make private any content formerly associated with only this label.")
  ...
  def recursivelyUpdatePositionsAndParents(itemIds: List[AnyRef]) {
    itemIds.zipWithIndex.map(x => {
      ...
      if (oldPos != newPos || oldParentId != newParentId) {
        ...
        ContentTagDAO.get(objId).map(obj => {
          if (oldPos != newPos) {
  ...
  def deleteBranch(itemId: AnyRef) {
    ...
    ContentTagDAO.delete(obj)
  }
  
  private class TreeTableSortDropHandler(tree: LabelTreeTable) extends DropHandler {
  ...
  def addContentTags(objs: List[ContentTag], parentId: AnyRef) {
    objs.map(obj => {
      ...
      this.getContainerProperty(itemId, "abbr").setValue(obj.getAbbr())
      this.getContainerProperty(itemId, "foreground").setValue(obj.getForeground())
      this.getContainerProperty(itemId, "background").setValue(obj.getBackground())
      ...
      if (obj.hasChildren()) {
        ...
        this.addContentTags(obj.getChildren(), itemId)
      } else {
