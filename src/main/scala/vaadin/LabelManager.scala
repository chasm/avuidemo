...
class LabelManager extends HorizontalLayout with Fragmented {
  
  def getFragment() = "labels"
  setSpacing(true)
  setMargin(true)
  setWidth("100%")
  ...
  colCenter.addComponent(new LabelPane())
...
object ContentTagContainer {
	def load(userId: String): Option[ContentTagContainer] = {
	  ContentTagDAO.getAllByUserId(userId) match {
	    case Nil => None
