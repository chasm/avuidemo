...
trait WorkManager[Task] {
  ...
  def manage( tasks : Seq[Task] ) : Seq[Wkr] = {
    for ( task <- tasks )
    yield {      
  ...
  def workIt( workforce : Seq[Wkr] )( k : Unit => Unit ) : Unit = {
    for( w <- workforce ) {
      w ! BeginWork( k )
    }
  }
...
) extends WorkManager[Task] with WorkLog[Task]
{
  ...
  def select( tasks : Seq[Task] ) : Option[Task] = {
    reset{      
...
class ChoiceExtractors[Task] {
  def unapply( choice : Choice[Task] )
  : ( Seq[Worker[Task,Choice]] )
  = {
    ( choice.workers.asInstanceOf[Seq[Worker[Task,Choice]]] )
  }
}
...
) extends WorkManager[A] with WorkLog[A]
{
  ...
  override def manage( tasks : Seq[A] ) : Seq[Wkr] = {
  ...
  override def workIt( wrkfrc : Seq[Wkr] )( k : Unit => Unit )
  : Unit = {
  ...
  def select( tasks : Seq[A] ) : Option[Seq[A]] = {
    reset{
      monitor.openMonitoringSession( this )
      shift {
	( k : Unit => Unit ) =>
...
class ModChoice(
  ...
  override def workers = cworkers.asInstanceOf[Seq[Wkr]]
